...
class TransE:
    def __init__(self, entityList, relationList, tripleList, margin = 1, learingRate = 0.01, dim = 50, L1 = True):
        self.margin = margin
    ...
    def transE(self, cI = 100):
        print("训练开始")
        for cycleIndex in range(cI):
            Sbatch = self.getSample(800)
            ...
            for sbatch in Sbatch:
                ...
                if(tripletWithCorruptedTriplet not in Sbatch):
                #if(tripletWithCorruptedTriplet not in tripleList):
                #if (tripletWithCorruptedTriplet not in Sbatch and tripletWithCorruptedTriplet not in Tbatch):
                    Tbatch.append(tripletWithCorruptedTriplet)
            self.update(Tbatch)
            if cycleIndex % 100 == 0:
                print("第%d次循环"%cycleIndex)
                print(self.loss)
                self.writeRelationVector(self.path+"/result/relationVector.txt")
                self.writeEntilyVector(self.path+"/result/entityVector.txt")
                self.loss = 0
                ...
                if self.L1:
                    ...
                    for i in range(self.dim):#不知道有没有pythonic的写法（比如列表推倒或者numpy的函数）？
                        ...
                        if tempNegtative[i] >= 0:
                            ...
                            tempNegtativeL1.append(-1)
                    tempPositive = array(tempPositiveL1)*self.learingRate
                    tempNegtative = array(tempNegtativeL1)*self.learingRate
...
if __name__ == '__main__':
    dirEntity = "entity2id.txt"
    entityIdNum, entityList = openDetailsAndId(dirEntity)
    dirRelation = "relation2id.txt"
    ...
    transE.writeRelationVector(transE.path+"/result/relationVector.txt")
    transE.writeEntilyVector(transE.path+"/result/entityVector.txt")
